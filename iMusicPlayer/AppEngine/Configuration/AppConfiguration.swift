...
//                    let personClass: AnyClass? = NSClassFromString(moduleName + "." + "Person")
                    if let cls = NSClassFromString(moduleName + ".\(vcs)") {
                        if let vcType = cls as? UIViewController.Type {
                            ...
                            VCN3.tabBarItem.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Color.md_ThemeColor] , for: UIControl.State.selected)
                            VCN3.tabBarItem.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: UIColor.init(rgba: "#888888")] , for: UIControl.State.normal)
                            vcsArr.append(VCN3)
                        }
                    }
