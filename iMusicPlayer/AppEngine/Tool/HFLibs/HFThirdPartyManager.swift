...
//                                          annotation: annotation)
    ...
    func application(app: UIApplication, openURL url: URL, options: [UIApplication.OpenURLOptionsKey : Any] = [:]) -> Bool {
        
        if url.scheme == HFThirdPartyManager.WXAppkey {
            return WXApi.handleOpen(url, delegate: HFThirdPartyManager.shared)
        }
        
        if (options[UIApplication.OpenURLOptionsKey.sourceApplication] as! String) == "com.tencent.mqq" {
        ...
        if let img = model.data_share?.data_share_img {
            KingfisherManager.shared.downloader.downloadImage(with: URL.init(string: img)!, retrieveImageTask: nil, options: nil, progressBlock: nil) {(image, error, url, data) in
                if finishBlock != nil && image != nil {
                    finishBlock!((image?.jpegData(compressionQuality: 0.5))!)
                }else {
                    if finishBlock != nil {
                        finishBlock!(#imageLiteral(resourceName: "password_close@2x.png").jpegData(compressionQuality: 0.5)!)
                    }
                }
            }
        }else {
