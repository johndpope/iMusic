...
class HFAlertController: UIAlertController, HFAlertBkViewDelegate {
    ...
    /// - Returns: 一个默认的提示框
    open class func alertController(title: String, message: String, positiveTitle: String = "确定", negativeTitle: String = "取消", preferredStyle: UIAlertController.Style = .alert, ConfirmCallBack:(() -> Void)?, CancelCallBack:(() -> Void)?) -> HFAlertController {
...
//        let cancelAction = UIAlertAction(title: "取消", style: UIAlertActionStyle.cancel, handler: nil)
        ...
        let yesAction = UIAlertAction(title: positiveTitle, style: UIAlertAction.Style.default) { (_) in
            if ConfirmCallBack != nil {
                ConfirmCallBack!()
            }
        }
    ...
    ///   - ConfirmCallBack: 确定按钮回调
    /// - Returns: 一个默认的提示框
    open class func alertController(config: MDAlertConfig, preferredStyle: UIAlertController.Style = .alert, ConfirmCallBack:(() -> Void)?, CancelCallBack:(() -> Void)?) -> HFAlertController {
        ...
        let yesAction = UIAlertAction(title: config.positiveTitle, style: UIAlertAction.Style.default) { (_) in
            if ConfirmCallBack != nil {
                ConfirmCallBack!()
            }
        }
        ...
        alertControllerStr.addAttribute(NSAttributedString.Key.foregroundColor, value: config.titleColor, range: NSRange(location: 0, length: config.title.count))
        ...
        alertControllerMessageStr.addAttribute(NSAttributedString.Key.foregroundColor, value: config.titleColor, range: NSRange(location: 0, length: config.desc.count))
