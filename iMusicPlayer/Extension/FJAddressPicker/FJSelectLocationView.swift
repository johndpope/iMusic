...
extension FJSelectLocationView {
    ...
    func setupUI() {
        ...
        button.setImage(UIImage.init(named: "icon_deleted"), for: UIControl.State.normal)
        button.contentMode = UIView.ContentMode.redraw
        button.addTarget(self, action:#selector(deletedButtonClick),for:UIControl.Event.touchUpInside)
        addSubview(button)
...
//        }
    ...
    func widthForsize(text:String, size:CGSize,font:CGFloat) -> CGFloat{
        let attrbute = [NSAttributedString.Key.font:UIFont.systemFont(ofSize: font)]
