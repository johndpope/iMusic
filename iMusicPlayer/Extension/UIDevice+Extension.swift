...
extension UIDevice {
    ...
    ///
    /// - Returns: -
    class func getLaunchSystemTime() -> Double {
        let timer = ProcessInfo.init().systemUptime
        QYTools.shared.Log(log: timer.description)
    ...
    ///
    /// - Returns: -
    class func getAppInstallTime() -> Double {
        var installTime: Double = 0.0
        ...
        do {
            ...
            //获取文件信息
            if let firstData = fileInfo[FileAttributeKey.creationDate] as? NSDate {
                //获取文件创建
                QYTools.shared.Log(log: firstData.description)
                
                installTime = Date().timeIntervalSince1970 - firstData.timeIntervalSince1970
            }
        }catch {
