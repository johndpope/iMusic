...
extension String {
    ...
    func md_setLabelColorByCount(count: Int, leftColor: UIColor, rightColor: UIColor) -> NSMutableAttributedString {
        ...
        strAttr.addAttribute(NSAttributedString.Key.foregroundColor, value: leftColor, range: range)
        //rightColor
        let range2 = NSMakeRange(count, self.characters.count-count)
        strAttr.addAttribute(NSAttributedString.Key.foregroundColor, value: rightColor, range: range2)
        return strAttr
    }
}
...
//    }
    ...
    func md_heightWithFontSize(_ fontSize: CGFloat, width: CGFloat) -> CGFloat {
        let attrs = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: fontSize)]
...
extension NSAttributedString{
    ...
    class  func strForRichText(text: String, colorText: [String], colors: [UIColor], fontSizes: [UIFont]) -> NSAttributedString {
        ...
        for index in 0..<colorText.count {
            ...
            //颜色处理
            attrstring.addAttribute(NSAttributedString.Key.foregroundColor, value:colors[index], range: theRange)
            attrstring.addAttribute(NSAttributedString.Key.font, value: fontSizes[index], range: theRange)
        }
